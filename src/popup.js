/**
 * Extension popup bridge — forwards chrome API calls to/from the hosted popup iframe.
 */
...
(function () {
  ...
  // Push storage changes into the iframe so it can react (e.g. reviews cache update).
  ...
  window.addEventListener('message', async (e) => {
    ...
    switch (type) {
      ...
      case 'STORAGE_SYNC_GET':
        try { reply(await chrome.storage.sync.get(payload.defaults)); }
        catch { reply(payload.defaults || {}); }
        break;
      case 'STORAGE_SYNC_SET':
        try { await chrome.storage.sync.set(payload.obj); } catch {}
        reply({ ok: true });
        break;
