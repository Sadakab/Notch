...
// src/supabase-client-key.js
function isClientSafeSupabaseKey(key) {
...
var init_supabase_client_key = __esm({
  "src/supabase-client-key.js"() {
  }
});
...
var init_supabase_config = __esm({
  "src/supabase-config.js"() {
    SUPABASE_URL = "https://qsecybzfpleplpqzsrza.supabase.co";
    SUPABASE_ANON_KEY = "sb_publishable_g3bQl0sI2ZsxAlTVysDhOA_3VPqRPc2";
  }
});
...
async function loadClipReviewRow(client, platform, clipId, clipOwnerUserId) {
  ...
  const r = await client.from("clip_reviews").select("*").eq("platform", platform).eq("user_id", clipOwnerUserId).eq("clip_id", clipId).maybeSingle();
...
async function syncAuthMarkerFromChromeStorage() {
  ...
  if ((!user || !user.email) && userBlob != null) {
    const parsed = typeof userBlob === "string" ? (() => {
      try {
...
function isUnauthorizedError(err) {
  ...
  return message.includes("jwt") && (message.includes("expired") || message.includes("invalid") || message.includes("malformed"));
}
...
async function handoffSupabaseSession(sessionPayload) {
...
async function cleanupRealtimeForTab(tabId) {
...
async function subscribeClipRealtimeForTab(tabId, platform, clipId, hostUserId) {
  ...
  await unsubscribeClipRealtimeByTabId(tabId);
  const channel = client.channel(`clip-reactions-${tabId}-${Date.now()}`).on(
    "postgres_changes",
    {
      event: "UPDATE",
      schema: "public",
...
function handleRuntimeMessage(msg, sendResponse, sender) {
    ...
    getSessionSafe(client).then(async ({ session, error: sessErr }) => {
      ...
      try {
        if (!isCollaboratorWrite) {
          const { data, error } = await client.from("clip_reviews").upsert(row, { onConflict: "user_id,platform,clip_id" }).select("id").single();
          if (error) {
            const errLine = formatSupabaseError(error);
        ...
        const { data: updatedRow, error: updateErr } = await client.from("clip_reviews").update({
          comments: commentsPayload,
          title: cloudOptionalTextField(msg.title),
          thumbnail_url: cloudOptionalTextField(msg.thumbnailUrl),
          updated_at: row.updated_at
        }).eq("id", existingRow.id).eq("user_id", rowUserId).eq("platform", platform).eq("clip_id", existingRow.clip_id).select("id").single();
        if (updateErr) {
  ...
  if (msg?.type === "MF_CLOUD_LIST_CLIPS") {
    ...
    getSessionSafe(client).then(({ session, error: sessErr }) => {
      ...
      client.from("clip_reviews").select("*").order("updated_at", { ascending: false }).then(({ data, error }) => {
        if (error) {
          sendResponse({ ok: false, items: [] });
          return;
        }
...
async function restoreAuthMarker() {
  try {
    await syncAuthMarkerFromChromeStorage();
    const client = getSupabase();
    if (!client) return;
...
var init_sw_cloud = __esm({
  "src/sw-cloud.js"() {
    init_dist4();
    init_supabase_client_key();
    init_supabase_config();
    AUTH_STORAGE_KEY = "sb-notch-auth";
    AUTH_STATE_KEY = "markframe_auth_state";
    AUTH_CONFIRM_URL = `https://notch.video/auth/confirm.html?extid=${chrome.runtime.id}`;
...
// src/sw-main.js
var require_sw_main = __commonJS({
  "src/sw-main.js"() {
    init_supabase_client_key();
    init_supabase_config();
    init_sw_cloud();
    var CLOUD_TYPES = /* @__PURE__ */ new Set([
    ...
    var NOTCH_PENDING_SHARE_STORAGE_KEY = "notch_pending_share";
    ...
    var POPUP_TYPES = /* @__PURE__ */ new Set([
      "MF_SUPABASE_CONFIG",
      "MF_OPEN_TAB",
    ...
    function handlePopupMessage(msg, sendResponse, sender) {
      ...
      if (type === "FETCH_VIMEO_OEMBED_THUMB" && msg.clipId) {
        ...
        fetch(api).then((r) => r.ok ? r.json() : Promise.reject(new Error(String(r.status)))).then((j) => {
    ...
    chrome.runtime.onMessageExternal.addListener((msg, _sender, sendResponse) => {
      if (msg?.type === "NOTCH_STORE_PENDING_SHARE") {
        ...
        if (uidRaw2 == null || String(uidRaw2).trim() === "" || platformRaw2 == null || String(platformRaw2).trim() === "" || clipRaw2 == null || String(clipRaw2).trim() === "") {
          sendResponse({ ok: false, error: "invalid_args" });
          return false;
        }
        const pending = {
          uid: String(uidRaw2).trim(),
          platform: String(platformRaw2).trim(),
          clip: String(clipRaw2),
          receivedAt: Date.now()
        };
        void chrome.storage.local.set({ [NOTCH_PENDING_SHARE_STORAGE_KEY]: pending }).then(() => sendResponse({ ok: true })).catch((e) => sendResponse({ ok: false, error: String(e?.message || e) }));
        return true;
      }
...
export default require_sw_main();
