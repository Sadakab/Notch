(() => {
  ...
  var require_popup = __commonJS({
    "src/popup.js"() {
      var PREFS_STORAGE_KEY = "notch_prefs";
      var SYNC_KEY_AUTO_SHOW_SIDEBAR = "autoShowSidebar";
      ...
      var AVATAR_DATA_URL_MAX_LEN = 7e3;
      var COMPANY_LOGO_DATA_URL_MAX_LEN = 48e3;
      var POPUP_CACHE_KEY_CONFIG = "notch_supabase_config";
      var POPUP_CACHE_KEY_USER = "notch_popup_user";
      var POPUP_CACHE_KEY_REVIEWS = "notch_popup_reviews";
      var POPUP_CACHE_KEY_SETTINGS = "notch_popup_settings";
      var settingsSession = {
        user: (
          /** @type {{ id?: string; email?: string; plan?: string; billingPortalUrl?: string } | null} */
          null
        ),
        pro: false
      };
      var changeEmailCooldownUntil = 0;
      var changeEmailCooldownTimer = null;
      var proPollTimer = null;
      var proPollDeadline = 0;
      function normalizeUuidForCompare(u) {
        return String(u || "").trim().toLowerCase().replace(/-/g, "");
      }
      ...
      function normalizePreferences(raw) {
        ...
        return {
          ...
          avatar: typeof avatarRaw === "string" && normalizeCommentAvatarUrl(avatarRaw) ? normalizeCommentAvatarUrl(avatarRaw) : null,
          logoDataUrl: typeof logoRaw === "string" && normalizeCommentAvatarUrl(logoRaw) ? normalizeCommentAvatarUrl(logoRaw) : null,
        ...
        return new Promise((resolve, reject) => {
          ...
          r.onload = () => {
            ...
            img.onload = () => {
              ...
              const maxLen = Math.max(1e3, Number(opts?.maxLen) || AVATAR_DATA_URL_MAX_LEN);
      ...
      async function loadSettingsView() {
        ...
        if (!cfg?.configured) {
          showEl(warn, true);
          warn.textContent = "Add your Supabase key in src/supabase-config.js, run npm run build, and reload the extension.";
          setSignedInSectionsVisible(false);
          const prefs2 = await loadCachedPreferences();
          fillSettingsForm(prefs2, "");
          document.getElementById("np-timestamp-format").value = prefs2.timestampFormat === "long" ? "long" : "short";
        ...
        if (isUsablePopupSettingsCache(cacheEntry)) {
          const prefs2 = normalizePreferences({ ...PREFERENCE_DEFAULTS, ...cacheEntry.prefs });
          const cachedUser = cacheEntry.user;
          const user2 = cachedUser && cachedUser.id ? {
            id: cachedUser.id,
            email: cachedUser.email,
            plan: cachedUser.plan,
            billingPortalUrl: cachedUser.billingPortalUrl
          } : null;
          const pro2 = user2 ? String(user2.plan || "").trim().toLowerCase() === "pro" : false;
          settingsSession = { user: user2, pro: pro2 };
          const prevFp = settingsViewFingerprint(user2, prefs2, pro2);
          await applySettingsViewUi(user2, prefs2, pro2);
      ...
      function startProPoll() {
        stopProPoll();
        proPollDeadline = Date.now() + 2 * 60 * 1e3;
      ...
      function updateChangeEmailButton() {
        ...
        btn.textContent = `Change (${Math.ceil(remainingMs / 1e3)}s)`;
        btn.disabled = true;
      }
      function startChangeEmailCooldown() {
        changeEmailCooldownUntil = Date.now() + 60 * 1e3;
        updateChangeEmailButton();
        clearChangeEmailCooldownTimer();
        changeEmailCooldownTimer = window.setInterval(updateChangeEmailButton, 1e3);
      }
      ...
      function wireSettingsHandlers() {
        ...
        document.getElementById("np-profile-avatar-file")?.addEventListener("change", () => {
          ...
          compressAvatarFile(f).then(async (dataUrl) => {
        ...
        document.getElementById("np-logo-file")?.addEventListener("change", () => {
          ...
          compressCompanyLogoFile(f).then(async (dataUrl) => {
      ...
      function parseYoutubeVideoIdFromUrl(urlStr) {
        ...
        try {
          ...
          const ytHosts = /* @__PURE__ */ new Set([
            "www.youtube.com",
            "youtube.com",
            "m.youtube.com",
            "music.youtube.com",
      ...
      function parseVimeoClipIdFromUrl(urlStr) {
        ...
        try {
          ...
          if (isPlayer) {
            const vi2 = parts.indexOf("video");
            if (vi2 >= 0 && parts[vi2 + 1] && /^\d+$/.test(parts[vi2 + 1])) return parts[vi2 + 1];
      ...
      function parseLoomClipIdFromUrl(urlStr) {
        ...
        try {
          const u = new URL(urlStr);
          const h = u.hostname.toLowerCase();
          const onLoom = h === "loom.com" || h === "www.loom.com" || h.length > 10 && h.endsWith(".loom.com");
      ...
      function isSupportedNotchVideoUrl(urlStr) {
        ...
        try {
          ...
          if ((h === "dropbox.com" || h === "www.dropbox.com" || h === "m.dropbox.com") && isDropboxShareViewerPathname(u.pathname)) {
            return true;
          }
          return false;
        } catch {
      ...
      async function writePopupCache(cfg, user, listPayload) {
        const cfgObj = cfg && typeof cfg === "object" ? { ok: true, configured: !!cfg.configured, url: String(cfg.url || "") } : { ok: true, configured: false, url: "" };
        ...
        const hasSignedInList = !!uid && listPayload && typeof listPayload === "object" && listPayload.ok === true && Array.isArray(listPayload.items);
        if (hasSignedInList) {
          await chrome.storage.local.set({
            ...base,
      ...
      function updateNpGateCopy(supabaseConfigured) {
        ...
        if (!supabaseConfigured) {
          ...
          msgEl.textContent = "Add your Supabase URL and publishable (anon) API key to src/supabase-config.js, run npm run build, then reload this extension in chrome://extensions.";
          showEl(form, false);
        } else {
